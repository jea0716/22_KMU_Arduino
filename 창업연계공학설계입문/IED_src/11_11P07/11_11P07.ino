...
int count = 0; 
float arr[10] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100};
...
void loop() {
  ...
  // wait until next sampling time. 샘플링 주기를 맞춰주는 역할
  if (millis() < (last_sampling_time + INTERVAL))
    return;

  dist_raw = USS_measure(PIN_TRIG, PIN_ECHO); // read distance 초음파센서 주고받는거
  ...
  // add your code here! 180, 220 을 관계를 보고 조건문으로 작성
  // Use _TARGET_LOW, _TARGTE_HIGH
  // 
  int _DUTY_NUM = _DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN) / 180) * (int(dist_ema) - 180);
  if(dist_raw < _TARGET_LOW) myservo.writeMicroseconds(_DUTY_MIN);
  else if(dist_raw >= _TARGET_LOW && dist_raw <= _TARGET_HIGH) myservo.writeMicroseconds(_DUTY_NUM);
  else if(dist_raw > _TARGET_HIGH) myservo.writeMicroseconds(_DUTY_MAX);
